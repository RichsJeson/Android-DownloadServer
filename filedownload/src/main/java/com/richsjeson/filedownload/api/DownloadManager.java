...
public class DownloadManager {
	...
	private static final String[] COLUMNS = new String[] {
			...
			COLUMN_LOCAL_URI, COLUMN_STATUS,
			COLUMN_REASON,
			Downloads.COLUMN_ERROR_CODE,
	...
	private static final Set<String> LONG_COLUMNS = new HashSet<String>(
			Arrays.asList(COLUMN_ID, COLUMN_TOTAL_SIZE_BYTES, COLUMN_STATUS,
					COLUMN_REASON,Downloads.COLUMN_ERROR_CODE,COLUMN_BYTES_DOWNLOADED_SO_FAR,
					COLUMN_LAST_MODIFIED_TIMESTAMP));
...
//			}
	...
	public void pauseDownload(long... ids) {
		...
		try {
			for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor
				...
				if (status != STATUS_RUNNING && status != STATUS_PENDING && translateStatus(status) != STATUS_FAILED) {
					throw new IllegalArgumentException(
	...
	private static class CursorTranslator extends CursorWrapper {
		...
		private long translateLong(String column) {
			...
			if( column.equals(Downloads.COLUMN_ERROR_CODE)){
				return (int) getUnderlyingLong(Downloads.COLUMN_ERROR_CODE);
			}
